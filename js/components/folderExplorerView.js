...
import { formatBytes, showToast, CONFIG } from '../config.js';
...
export class FolderExplorerView {
  ...
  renderFolderContent() {
    ...
    filesToShow.forEach((fileItem) => {
      ...
      const thumb = fileItem.thumbUrl || fileItem.url || (fileItem.fileBlob ? URL.createObjectURL(fileItem.fileBlob) : '');
      const isVideo = fileItem.mediaType === 'video';

      card.innerHTML = `
        ${isVideo ? `<video src="${thumb}" class="folder-file-thumb" muted></video>` : `<img src="${thumb}" class="folder-file-thumb" alt="${fileItem.fileName}">`}
