...
import { formatDate, formatBytes, showToast } from '../config.js';
...
export class TripsView {
  ...
  async render() {
    ...
    this.trips.forEach((trip) => {
      ...
      const coverPhoto = tripPhotos.find(m => m.id === trip.coverPhotoId) || tripPhotos[0] || {};
      const coverThumb = coverPhoto.thumbUrl || coverPhoto.url || (coverPhoto.fileBlob ? URL.createObjectURL(coverPhoto.fileBlob) : '');
      ...
      card.innerHTML = `
        <div class="trip-cover-wrap">
          ${coverThumb ? `<img src="${coverThumb}" alt="${trip.name}" class="trip-cover-img" loading="lazy">` : `<div class="trip-cover-placeholder"><iconify-icon icon="lucide:image"></iconify-icon></div>`}
