  ...
  initElements() {
    ...
    if (savedFolderLink && this.inputFolderLink) {
      ...
      if (this.labelLinkStatus) {
        this.labelLinkStatus.style.display = 'inline-flex';
        this.labelLinkStatus.textContent = `✓ Google Drive Folder Connected`;
      }
    }
    ...
    if (this.btnSaveFolderLink && this.inputFolderLink) {
      this.btnSaveFolderLink.addEventListener('click', (e) => {
        ...
        try {
          ...
          if (this.labelLinkStatus) {
            this.labelLinkStatus.style.display = 'inline-flex';
            this.labelLinkStatus.textContent = `✓ Connected (Folder: ${res.folderId ? res.folderId.slice(0, 10) + '...' : 'Root'})`;
          }
          this.app.navbar.updateStorageIndicator();
        } catch (err) {
          showToast(err.message, 'danger');
